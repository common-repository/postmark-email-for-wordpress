 ...
 *
 * Modified by Joost de Valk to use WP HTTP API instead of CURL.
 *
...
class Mail_Postmark
{
	...
	*/
	public function __construct()
	{
		...
		if (defined('POSTMARKAPP_MAIL_FROM_ADDRESS')) {
			$from = explode(',',POSTMARKAPP_MAIL_FROM_ADDRESS);
	...
	*/
	...
	/**
	* Specify certificate path for SSL validation
	* 
	* @param string $handle Resource ID of the handle to specify the certificate for
	*/
	public function curl_ca_file( $handle ) {
		curl_setopt($handle, CURLOPT_CAINFO, POSTMARKAPP_PLUGIN_DIR_PATH . 'Postmark/cacert.pem');
	}
	...
	*/
	public function send()
	{
		...
		add_action( 'http_api_curl', array( $this, 'curl_ca_file') );
		$return = wp_remote_post( 'https://api.postmarkapp.com/email', array(
				'body' 		=> json_encode($data),
				'headers'	=> $headers,
				'sslverify'	=> true	
			)
		);
		remove_action( 'http_api_curl', array( $this, 'curl_ca_file') );
		...
		$httpCode = (int) $return['response']['code'];
		...
		if ( $httpCode != 200 ) {
			if ($httpCode == 422) {
				...
				echo '<pre>'.print_r(json_decode($return['body']),1).'</pre>';
				return false;
			}
		}
